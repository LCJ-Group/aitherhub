...
async def transcribe_clip(
    ...
    try:
        ...
        for seg in segments:
            ...
            # ⚠️ PROTECTED: Japanese word-boundary-aware splitting.
            # Whisper returns Japanese text as individual characters in word-level timestamps.
            # This logic accumulates characters and prefers splitting AFTER particles/conjunctions
            # to avoid breaking words like "コンセ|ントレート" or "フォ|ーム".
            # DO NOT revert to simple character-count splitting.
            if seg_words:
                # Japanese particles and conjunctions that make good break points
                _JP_PARTICLES = set('はがをにでともやかねよなへのてた')
                _JP_CONJUNCTIONS = ['して', 'って', 'ので', 'から', 'けど', 'ても', 'たら', 'ながら', 'だけど', 'ですが']
                _JP_PUNCTUATION = set('、。！？，')

                current_text = ""
                current_words = []
                current_start = None
                # Track the best break point seen within the current chunk
                best_break_idx = -1  # index in current_words after which to break
                best_break_priority = 0  # 3=punctuation, 2=conjunction, 1=particle

                def _flush_segment(txt, words, start_t):
                ...
                for w in seg_words:
                    ...
                    # Check if this word ends at a natural break point
                    break_priority = 0
                    if w_text in _JP_PUNCTUATION or (len(w_text) > 0 and w_text[-1] in _JP_PUNCTUATION):
                        break_priority = 3
                    elif any(test_text.endswith(conj) for conj in _JP_CONJUNCTIONS):
                        break_priority = 2
                    elif w_text in _JP_PARTICLES:
                        break_priority = 1

                    # Must break: exceeding limits
                    if (len(test_text) > MAX_CHARS_PER_LINE or test_duration > MAX_DURATION_PER_LINE) and len(current_text) >= MIN_CHARS_PER_LINE:
                        # If we have a good break point within the current chunk, use it
                        if best_break_idx >= 0 and best_break_idx < len(current_words) - 1:
                            # Split at the best break point
                            break_words = current_words[:best_break_idx + 1]
                            break_text = "".join(bw.get("word", "").strip() for bw in break_words)
                            _flush_segment(break_text, break_words, current_start)
                            # Remaining words + current new word become the new chunk
                            remaining_words = current_words[best_break_idx + 1:]
                            remaining_words.append(w)
                            current_words = remaining_words
                            current_text = "".join(rw.get("word", "").strip() for rw in current_words)
                            current_start = current_words[0].get("start", w_start)
                        else:
                            # No good break point found, break at current position
                            _flush_segment(current_text, current_words, current_start)
                            current_text = w_text
                            current_words = [w]
                            current_start = w_start
                        best_break_idx = -1
                        best_break_priority = 0
                    else:
                        current_text = test_text
                        current_words.append(w)
                        # Update best break point if this is a good one
                        if break_priority > best_break_priority and len(current_text) >= MIN_CHARS_PER_LINE:
                            best_break_idx = len(current_words) - 1
                            best_break_priority = break_priority
                ...
                # Strategy 2: No word timestamps - split by character count with Japanese awareness
                # ⚠️ PROTECTED: Uses same particle/conjunction break logic as Strategy 1.
                chars = list(text_val)
                ...
                if total_chars <= MAX_CHARS_PER_LINE:
                    ...
                    _PARTICLES_S2 = set('はがをにでともやかねよなへのてた')
                    _PUNCT_S2 = set('、。！？，')
                    _CONJ_S2 = ['して', 'って', 'ので', 'から', 'けど', 'ても', 'たら', 'ながら']
                    # Find break points
                    break_pts = []
                    for ci in range(1, total_chars - 1):
                        ...
                        if ch in _PUNCT_S2:
                            ...
                            for conj in _CONJ_S2:
                                conj_chars = list(conj)
                                if ci + len(conj_chars) <= total_chars:
                                    if chars[ci:ci + len(conj_chars)] == conj_chars:
                                        pri = 2
                                        break
                            if pri == 0 and ch in _PARTICLES_S2 and ci + 1 < total_chars and chars[ci + 1] not in _PARTICLES_S2:
                                pri = 1
                        if pri > 0:
                            break_pts.append((ci + 1, pri))
                    # Select breaks near ideal positions
                    num_chunks = max(1, -(-total_chars // MAX_CHARS_PER_LINE))
                    ideal_size = -(-total_chars // num_chunks)
                    selected = [0]
                    last_brk = 0
                    for tgt in range(ideal_size, total_chars, ideal_size):
                        best_pos, best_pri, best_dist = None, 0, float('inf')
                        search = int(ideal_size * 0.4)
                        for bp_pos, bp_pri in break_pts:
                            if bp_pos <= last_brk or bp_pos >= total_chars - 2:
                                continue
                            d = abs(bp_pos - tgt)
                            if d <= search and (bp_pri > best_pri or (bp_pri == best_pri and d < best_dist)):
                                best_pos, best_pri, best_dist = bp_pos, bp_pri, d
                        if best_pos is not None:
                            ...
                            selected.append(tgt)
                            last_brk = tgt
                    selected.append(total_chars)
                    for si in range(len(selected) - 1):
                        chunk_text = "".join(chars[selected[si]:selected[si + 1]])
                        if not chunk_text.strip():
                            continue
                        ratio_start = selected[si] / total_chars
                        ratio_end = selected[si + 1] / total_chars
                        split_segments.append({
                            "start": round(seg_start + seg_duration * ratio_start, 3),
                            "end": round(seg_start + seg_duration * ratio_end, 3),
                            "text": chunk_text,
                        })
...
async def _run_export_job_inner(job_id: str, video_id: str, clip_url: str, captions, style: str,
    ...
    try:
        ...
        # ── Step 1c: Adjust caption timestamps for deleted segments ──
        # ⚠️ PROTECTED: When scenes are deleted (split_segments with enabled=false),
        # the video is shorter after concat. Captions must be remapped to the new timeline.
        # Without this, subtitles appear at wrong times in the exported video.
        if split_segments:
            all_segs_sorted = sorted(split_segments, key=lambda s: s.get('start', 0) if isinstance(s, dict) else s.start)
            disabled_segs = [s for s in all_segs_sorted if not (s.get('enabled', True) if isinstance(s, dict) else s.enabled)]
            if disabled_segs:
                logger.info(f"[export-job {job_id}] Adjusting caption timestamps for {len(disabled_segs)} deleted segments")
                adjusted_captions = []
                for cap in captions:
                    cap_dict = cap.dict() if hasattr(cap, 'dict') else dict(cap)
                    cap_start = float(cap_dict.get('start', 0))
                    cap_end = float(cap_dict.get('end', 0))
                    # Skip captions entirely inside a deleted range
                    in_deleted = False
                    for ds in disabled_segs:
                        ds_start = float(ds.get('start', 0) if isinstance(ds, dict) else ds.start)
                        ds_end = float(ds.get('end', 0) if isinstance(ds, dict) else ds.end)
                        if cap_start >= ds_start and cap_end <= ds_end:
                            in_deleted = True
                            break
                    if in_deleted:
                        continue
                    # Calculate cumulative deleted time before this caption
                    deleted_before = 0.0
                    for ds in disabled_segs:
                        ds_start = float(ds.get('start', 0) if isinstance(ds, dict) else ds.start)
                        ds_end = float(ds.get('end', 0) if isinstance(ds, dict) else ds.end)
                        if ds_end <= cap_start:
                            deleted_before += (ds_end - ds_start)
                        elif ds_start < cap_start:
                            deleted_before += (cap_start - ds_start)
                    cap_dict['start'] = round(cap_start - deleted_before, 3)
                    cap_dict['end'] = round(cap_end - deleted_before, 3)
                    adjusted_captions.append(cap_dict)
                logger.info(f"[export-job {job_id}] Adjusted {len(adjusted_captions)} captions (removed {len(captions) - len(adjusted_captions)} in deleted ranges)")
