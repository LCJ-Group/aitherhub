...
@router.post("/debug/test-vision", summary="Test GPT Vision analysis (debug)")
async def debug_test_vision(_auth: bool = Depends(verify_admin_key)):
    """Test GPT Vision analysis on a known product image."""
    import os, openai, json as _json

    results = {}
    image_url = "https://p16-oec-sg.ibyteimg.com/tos-alisg-i-aphluv4xwc-sg/3190347f2665423abb8857fe40c857a0~tplv-aphluv4xwc-resize-webp:800:800.webp"
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {"role": "user", "content": [
                    {"type": "text", "text": 'What product is shown? Reply JSON: {"product": "..."}'},
        ...
        results["vision_direct"] = "SUCCESS"
        results["vision_response"] = response.choices[0].message.content.strip()[:300]
    except Exception as e:
    ...
    try:
        ...
        vision_data = await _analyze_product_image_vision(
            image_url, "KYOGOKU パーソナルカラーシャンプー", "ja"
        )
        results["vision_function"] = "SUCCESS" if vision_data else "EMPTY"
        results["vision_keys"] = list(vision_data.keys()) if vision_data else []
        results["selling_points_count"] = len(vision_data.get("selling_points", []))
        results["achievements"] = vision_data.get("achievements", [])
    except Exception as e:
        results["vision_function"] = f"FAILED: {str(e)[:500]}"
