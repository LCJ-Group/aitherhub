"""
ElevenLabs Text-to-Speech Service for AitherHub

This module provides a Python client for the ElevenLabs TTS API, enabling
voice cloning in 32+ languages including Japanese. It is designed to work
in a hybrid architecture with Tencent Cloud Digital Human:

  Flow: Text → ElevenLabs TTS (cloned voice) → Audio → Tencent Cloud (lip sync)
...
class ElevenLabsTTSService:
    ...
    This service generates speech audio from text using a cloned voice,
    outputting PCM audio compatible with Tencent Cloud's audio driver.

    Usage:
        service = ElevenLabsTTSService()
        audio_bytes = await service.text_to_speech("こんにちは、皆さん！")
        chunks = service.chunk_audio_for_tencent(audio_bytes)
    """
