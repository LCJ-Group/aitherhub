...
async def lifespan(app: FastAPI):
    ...
    yield
    if engine and engine.state == EngineState.STREAMING:
        engine.stop_stream()
...
class PrepareRequest(BaseModel):
    ...
    bbox_shift: int = Field(0, description="Bounding box shift")
    extra_margin: int = Field(10, description="Extra margin for face cropping")


class StartRequest(BaseModel):
    rtmp_url: str = Field(..., description="RTMP destination URL (e.g., rtmp://live.twitch.tv/app/KEY)")
...
class SpeakRequest(BaseModel):
    text: Optional[str] = Field(None, description="Text to speak (will be converted to audio via TTS)")
    audio_url: Optional[str] = Field(None, description="URL to pre-generated audio file")
    audio_path: Optional[str] = Field(None, description="Local path to audio file on GPU Worker")
    voice_id: Optional[str] = Field(None, description="TTS voice ID (for ElevenLabs)")
...
class RespondRequest(BaseModel):
    ...
    product_info: Optional[str] = Field(None, description="Current product information for context")
...
@app.get("/api/v1/live/status", response_model=StatusResponse)
...
async def prepare_avatar(req: PrepareRequest, x_api_key: str = Header(None)):
    ...
    if engine.state not in (EngineState.IDLE, EngineState.ERROR):
        raise HTTPException(
            status_code=409,
            detail=f"Cannot prepare in state: {engine.state.value}"
        )
    ...
    # Run preparation (this takes time - model loading + video processing)
...
async def start_stream(req: StartRequest, x_api_key: str = Header(None)):
    ...
    if not engine.musetalk.models_loaded:
        raise HTTPException(
            status_code=400,
            detail="Models not loaded. Call /prepare first."
        )
...
async def speak(req: SpeakRequest, background_tasks: BackgroundTasks, x_api_key: str = Header(None)):
    """Make the AI avatar speak. Accepts text (TTS) or audio URL/path."""
    verify_api_key(x_api_key)
    if not engine:
        raise HTTPException(status_code=500, detail="Engine not initialized")
    ...
    if engine.state not in (EngineState.STREAMING, EngineState.SPEAKING):
        raise HTTPException(
            status_code=409,
            detail=f"Cannot speak in state: {engine.state.value}"
        )
    ...
    if req.audio_path:
        ...
        raise HTTPException(
            status_code=400,
            detail="Provide text, audio_url, or audio_path"
        )

    # Generate lip-sync and inject into stream
...
async def respond_to_comment(req: RespondRequest, x_api_key: str = Header(None)):
    ...
    GPT generates response text → TTS → Lip-sync → Stream.
    """
    verify_api_key(x_api_key)
    if not engine:
        raise HTTPException(status_code=500, detail="Engine not initialized")
    ...
    if engine.state not in (EngineState.STREAMING, EngineState.SPEAKING):
        raise HTTPException(
            status_code=409,
            detail=f"Cannot respond in state: {engine.state.value}"
        )
    ...
    # Step 3: Generate lip-sync and inject
    start = time.time()
    success = engine.speak(audio_path)
...
async def stop_stream(x_api_key: str = Header(None)):
    ...
    if engine.state not in (EngineState.STREAMING, EngineState.SPEAKING):
        raise HTTPException(
            status_code=409,
            detail=f"Cannot stop in state: {engine.state.value}"
        )
