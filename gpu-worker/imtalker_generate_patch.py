"""
Patched IMTalker generate.py - saves crop coordinates to JSON for accurate compositing.
This file is deployed to /workspace/IMTalker/generator/generate.py on the GPU Worker pod.
"""
import os
import json
import datetime
...
class DataProcessor:
    def __init__(self, opt):
        ...
        # Store last crop coordinates for compositing
    ...
    def process_img(self, img: Image.Image) -> Image.Image:
        ...
        # ── Save crop coordinates for compositing ──
        self.last_crop_coords = {
            "x1": int(x1_new),
            "y1": int(y1_new),
            "x2": int(x2_new),
            "y2": int(y2_new),
        ...
        print(f"CROP_COORDS: {json.dumps(self.last_crop_coords)}")
...
class InferenceAgent:
    ...
    def save_video(self, vid_tensor, video_path, audio_path):
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as tmp:
            ...
            torchvision.io.write_video(temp_filename, vid, fps=int(self.opt.fps))
    ...
    def run_inference(self, res_path, ref_path, aud_path, pose_path=None, gaze_path=None, **kwargs):
        ...
        # ── Save crop coordinates JSON next to output video ──
        if self.data_processor.last_crop_coords:
            crop_json_path = os.path.splitext(res_path)[0] + "_crop_coords.json"
            with open(crop_json_path, 'w') as f:
                json.dump(self.data_processor.last_crop_coords, f)
            print(f"CROP_JSON: {crop_json_path}")
